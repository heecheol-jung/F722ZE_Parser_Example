...
static fl_bool_t is_msg_id_char(uint8_t data);
static fl_bool_t is_device_id_char(uint8_t data);
static uint8_t get_device_id(uint8_t* buf, uint16_t buf_size);
static fl_bool_t is_tail(uint8_t data);
...
FL_DECLARE(fl_status_t) fl_txt_msg_parser_parse_response_event(fl_txt_msg_parser_t* parser_handle, uint8_t data, fl_txt_msg_t* msg_handle)
{
  ...
  case FL_TXT_MSG_PARSER_RCV_STS_ERROR:
    if (is_tail(data) == FL_TRUE)
    {
      parser_handle->error = (uint8_t)atoi((char*)parser_handle->buf);
      parser_handle->receive_state = FL_TXT_MSG_PARSER_RCV_STS_TAIL;
      clear_receive_buffer(parser_handle);
      ret = FL_OK;
    }
...
FL_DECLARE(uint8_t) fl_txt_msg_parser_get_msg_id(uint8_t* buf, uint8_t buf_size)
{
  if (buf_size == 4)
  {
    if (strcmp(FL_TXT_RREG_STR, (const char*)buf) == 0)
    {
      return FL_MSG_ID_READ_REGISTER;
    }
    else if (strcmp(FL_TXT_WREG_STR, (const char*)buf) == 0)
    {
      return FL_MSG_ID_WRITE_REGISTER;
    }
  }
  else if (buf_size == 5)
  {
...
static fl_bool_t process_command_data(fl_txt_msg_parser_t* parser_handle)
{
  ...
  if (parser_handle->msg_id == FL_MSG_ID_READ_REGISTER)
  {
    if (parser_handle->arg_count < 1)
    {
      fl_read_reg_t* rreg = (fl_read_reg_t*)&parser_handle->payload;

      rreg->address = (uint32_t)atoi((const char*)parser_handle->buf);
      parser_handle->arg_count++;
      ret = FL_TRUE;
    }
  }
  else if (parser_handle->msg_id == FL_MSG_ID_WRITE_REGISTER)
  {
    fl_write_reg_t* wreg = (fl_write_reg_t*)&parser_handle->payload;
    if (parser_handle->arg_count == 0)
    {
      wreg->address = (uint32_t)atoi((const char*)parser_handle->buf);
      parser_handle->arg_count++;
      ret = FL_TRUE;
    }
    else if (parser_handle->arg_count == 1)
    {
      wreg->value= (uint32_t)atoi((const char*)parser_handle->buf);
      parser_handle->arg_count++;
      ret = FL_TRUE;
    }
  }
...
static fl_bool_t process_response_event_data(fl_txt_msg_parser_t* parser_handle)
{
  ...
  if (parser_handle->msg_id == FL_MSG_ID_READ_REGISTER)
  {
    fl_read_reg_resp_t* rreg = (fl_read_reg_resp_t*)&parser_handle->payload;
    if (parser_handle->arg_count == 0)
    {
      rreg->address = (uint32_t)atoi((const char*)parser_handle->buf);
      parser_handle->arg_count++;
    ...
    {
      rreg->value = (uint32_t)atoi((const char*)parser_handle->buf);
      parser_handle->arg_count++;
      ret = FL_TRUE;
    }
  }
