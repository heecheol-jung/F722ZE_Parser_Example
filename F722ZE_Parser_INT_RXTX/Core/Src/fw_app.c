...
FL_DECLARE(void) fw_app_init(void)
{
  ...
  fl_q_init(&g_app.proto_mgr.q, _g_q1_buf, sizeof(_g_q1_buf));
...
FL_DECLARE(void) fw_app_hw_init(void)
{
  ...
  HAL_UART_Receive_IT(&huart3, g_app.proto_mgr.rx_buf, 1);
}
...
static void int_transmit(void)
{
  ...
  ret = HAL_UART_Transmit_IT(&huart3, g_app.proto_mgr.out_buf, g_app.proto_mgr.out_length);
