  ...
  uint32_t packet_buf_len)
{
  ...
  case FL_MSG_ID_READ_REGISTER:
  {
    ...
    {
      len = sprintf((char*)packet_buf, "%s %ld,%d%c", fl_txt_msg_get_message_name(message_id), device_id, rreg->address, FL_TXT_MSG_TAIL);
    }
    break;
  }
  ...
  case FL_MSG_ID_WRITE_REGISTER:
  {
    ...
    {
      len = sprintf((char*)packet_buf, "%s %ld,%d,%d%c", fl_txt_msg_get_message_name(message_id), device_id, wreg->address, wreg->value, FL_TXT_MSG_TAIL);
    }
    break;
  }
    ...
    switch (message_id)
    {
      ...
      case FL_MSG_ID_READ_REGISTER:
      {
        ...
        if (arg_buf_len == sizeof(fl_read_reg_resp_t))
        {
          ...
          len = sprintf((char*)packet_buf, "%s %ld,%d,%d,%d%c", 
            fl_txt_msg_get_message_name(message_id), 
            device_id, 
            error, 
            rreg->address, 
